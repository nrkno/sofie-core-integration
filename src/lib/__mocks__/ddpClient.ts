...
const ddpSessions: Array<DDPClient> = []
...
class Socket extends EventEmitter {
	...
	public connect () {
		...
		console.log('Socket connect called', this._connectOk)
		if (this._connectOk) {
			orgSetTimeout(() => {
				console.log('Scoket emitting connect')
				this.emit('connect')
			}, 1)
		} else {
			orgSetTimeout(() => {
...
class Subscription {
	private _ddp: DDPClient
	private _collections: Collections = {}

	private _unsubscribed: boolean = false

	constructor (ddp: DDPClient) {
		this._ddp = ddp
	}
...
export class DDPClient extends EventEmitter {
	...
	// private static constructors: Array<Function>

	// static initialize () {
	// 	DDPClient.constructors = []
	// }

	// public _collections: Collections = {}

	private _mockCalls: {[methodName: string]: ((...attrs: any[]) => any) | null} = {}
	...
	public static mockConstructNext (cb: (ddp: DDPClient) => void) {
		constructors.push(cb)
		console.log('Added a constructor', constructors)
	}
	/*
	...
	*/
	public connect (onConnectCb?: (err?: Error | string) => void) {
		console.log('!"£$%^&*() Connections requested of mock client')
		...
		console.log('Host', this._mockHost, this.host, 'port', this._mockPort, this.port)
	...
	// }
	public mockCall (methodName: string, cb: ((...attrs: any[]) => any) | null) {
